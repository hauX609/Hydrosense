...
# Helper: repack repository to split large packs and retry pushing
function Repack-And-Push($remote, $branch, $retries = 3) {
    Write-Log "Repacking repository to split large pack files..."
    git repack -a -d --max-pack-size=50m 2>$null
    $attempt = 0
    while ($attempt -lt $retries) {
        $attempt++
        Write-Log "Attempting git push (try $attempt/$retries)..."
        git push $remote $branch
        if ($LASTEXITCODE -eq 0) {
            Write-Log "Push succeeded."
            return $true
        }
        Write-Log "Push failed (attempt $attempt). Retrying in 5s..."
        Start-Sleep -Seconds 5
    }
    Write-Log "Push failed after $retries attempts."
    return $false
}

# Try pushing any existing local commits first (helps when previous runs created many local commits)
Write-Log "Checking for pre-existing local commits to push..."
git fetch origin 2>$null
if (-not (Repack-And-Push 'origin' $Branch 3)) {
    Write-Log "Could not push existing commits; will continue and push batches as we commit new files."
}
